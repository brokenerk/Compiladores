	...
	def searchRule(self, symbol):
		...
		for rule in self.rules:
			if(rule.getSymbol() == symbol):
				rule.getNext().setPointBefore(True)
				listRules.append(deepcopy(rule))  
		return listRules
			...
			while(next != None):
				if(next.getPointBefore()):
					if(next.getSymbol() in self.noTerminals):
						...
						if(n != None):
							symbols = deepcopy(self.dpFirst[n.getSymbol()])
							if("epsilon" in symbols):
								symbols.remove("epsilon")
								symbols = symbols.union(prev)
						else:
